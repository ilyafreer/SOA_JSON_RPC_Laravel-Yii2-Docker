...
На первичный запрос клиента, сервер генерирует и отдает готовую форму
...
Клиент передает на сервер данные в формате JSON-RPC, предварительно проверяет токен страници клиента с токеном полученным при отправке формы.


Все контейнеры объеденены в общую сеть
Для запуска приложения необходимо в классе клиента JsonRpcClient указать ip сервера
Также проверить адрес подключения к базе
